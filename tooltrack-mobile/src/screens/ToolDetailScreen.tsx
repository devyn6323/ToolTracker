...
export function ToolDetailScreen({ route, navigation }: NativeStackScreenProps<AppStackParams, 'ToolDetail'>) {
  const { session } = useAuth(); const [tool, setTool] = useState<Tool>(); const [history, setHistory] = useState<ToolTransaction[]>([]); const [error, setError] = useState('');
...
const styles = StyleSheet.create({
  ...
  qrCard: { alignItems: 'stretch' }, qr: { alignItems: 'center', padding: 20, marginTop: 6 }, qrHint: { color: colors.muted, textAlign: 'center', lineHeight: 20 }, qrValue: { color: colors.blue, fontSize: 10, textAlign: 'center', marginTop: 9 },
