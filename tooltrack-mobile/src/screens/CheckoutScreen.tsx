...
const defaultDue = () => { const date = new Date(); date.setDate(date.getDate() + 7); return date.toISOString().slice(0, 10); };
export function CheckoutScreen({ route, navigation }: NativeStackScreenProps<AppStackParams, 'Checkout'>) {
  ...
  return <Screen><Card><Text style={styles.asset}>{tool.assetNumber}</Text><Text style={styles.name}>{tool.name}</Text><Text style={styles.person}>Checking out to {session?.user.name}</Text></Card><ErrorBanner message={error} />
    <Input label="Job or project" value={jobName} onChangeText={setJobName} placeholder="Warehouse Remodel" /><Input label="Use location" value={location} onChangeText={setLocation} placeholder="Job 42" /><Input label="Expected return date *" value={due} onChangeText={setDue} placeholder="YYYY-MM-DD" keyboardType="numbers-and-punctuation" />
